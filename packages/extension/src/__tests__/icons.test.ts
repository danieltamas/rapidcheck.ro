 ...
 *      filename's size).
 *   3. Every PNG is under the per-size budget.
...
const MAX_BYTES_BY_SIZE: Record<(typeof SIZES)[number], number> = {
  16: 2 * 1024,
  32: 3 * 1024,
  48: 5 * 1024,
  128: 15 * 1024,
...
describe('generated brand icons', () => {
  for (const size of SIZES) {
    ...
    it(`${filename} is under the per-size budget`, () => {
      const bytes = statSync(path).size;
      expect(bytes).toBeLessThanOrEqual(MAX_BYTES_BY_SIZE[size]);
  ...
  it('total PNG payload is reasonable (< 20 KB combined)', () => {
    ...
    // Hard ceiling above the current RapidCheck set — guards against accidental
