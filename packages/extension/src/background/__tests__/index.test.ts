...
describe('background SW — listener wiring', () => {
  it('registers exactly one listener on each event source (incl. onMessage)', () => {
    ...
    // Path C: SW only handles the popup's get-status request. The master
    // toggle is content-script-driven (storage subscription on the page).
...
describe('background SW — webNavigation.onCommitted', () => {
  ...
  it('clears the badge for an off-list domain (google.com)', async () => {
    await chromeStub.fire.committed({ tabId: 13, url: 'https://example.com/', frameId: 0 });
...
describe('background SW — runtime.onInstalled', () => {
  it('applies the badge to every open tab with a URL on install/update', async () => {
    ...
    chromeStub.tabsTable.set(47, { id: 47, url: 'https://example.com/' });
