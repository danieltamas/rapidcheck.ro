/**
 * @onegov/core must remain DOM-free.
 *
...
describe('@onegov/core has no DOM/browser-global side effects on import', () => {
  it('imports cleanly without referencing document/window/chrome', async () => {
    ...
    expect(typeof mod.applyPersonaOverrides).toBe('function');
    expect(typeof mod.validate).toBe('function');
    expect(typeof mod.loadBundled).toBe('function');
  });
