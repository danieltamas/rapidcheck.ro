/**
 * Root ESLint configuration for onegov.ro.
 *
 * Cross-package boundaries are enforced via overrides:
 *   - packages/core/**: no DOM, no chrome.*, no browser.*
 *   - packages/ui/**:   no chrome.*, no browser.* (DOM access only via given ShadowRoot)
...
module.exports = {
  ...
  overrides: [
    {
      files: ['packages/core/**/*.ts', 'packages/ui/**/*.ts', 'packages/ui/**/*.tsx'],
      rules: {
        'no-restricted-globals': [
          'error',
