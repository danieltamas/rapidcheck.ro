...
function main(): void {
  ...
  // Owner-provided canonical brand SVG (added 2026-05-02). Falls back to
  // brand-mark.svg if the explicit file isn't present (keeps the script
  // working in legacy worktrees).
  const brand = existsSync(resolve(srcDir, 'onegov.extension.svg'))
    ? readSvg('onegov.extension.svg')
